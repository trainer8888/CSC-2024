...
void Session::dissectTCP(std::span<uint8_t> buffer) {
  ...
  state.received_payload_length = payload.size();
...
int Session::encapsulateTCP(std::span<uint8_t> buffer, const std::string& payload) {
  ...
  // 對方的seq配我方的ack; 對方的ack配我方的seq; 我們的seq和ack是獨立的，互不相干
  // encapsulateTCP傳入的payload是我們輸入的內容，是hijacker的input，不是對方(server)的內容
  // 所以，我們的ack = 我們收到的packet的seq + 我們收到的packet的內容長度
  hdr.ack_seq = htonl(state.tcpseq + state.received_payload_length);
